...
public class AnchorBlockManager : MonoBehaviour
{
    ...
    public static void LoadAnchorBlocks(AnchorController anchor)
    {
        ...
        for (int i = 2; i < anchorBlocks.Count; i++) Destroy(anchorBlocks[i]);
        ...
        for (int i = 2; i < anchorConnectors.Count; i++) Destroy(anchorConnectors[i]);

        // create objects
        AnchorBlock[] blocks = AnchorManager.Instance.SelectedAnchor.Blocks.ToArray();

        for (int i = 0; i < blocks.Length; i++) blocks[i].CreateAnchorBlockObject(i > 2);
