...
internal class ForceSaveAllEditor : Editor
{
    [MenuItem("EDITORS/Force Save All")]
    private static void ForceSaveAll()
    {
        ...
        {
            if (string.IsNullOrEmpty(guids[i])) continue;

            Object asset = AssetDatabase.LoadAssetAtPath<Object>(AssetDatabase.GUIDToAssetPath(guids[i]));
