    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButton(1))
        {
            ...
            else
            {
                ...
                movement = new(MGame.PixelToUnit(movement.x), MGame.PixelToUnit(movement.y));
    ...
    private void Zoom(float zoomInput)
    {
        if (zoomInput != 0f && MMouse.Instance.OnScreen) // zoom
        {
            ...
            if (cam.orthographicSize + zoomInput * zoomSpeed >= minZoom && cam.orthographicSize + zoomInput * zoomSpeed <= maxZoom)
            {
                ...
                Vector2 newMouseOffset = MGame.PixelToUnit(prevMouseOffsetPixels, newOrthoSize);
