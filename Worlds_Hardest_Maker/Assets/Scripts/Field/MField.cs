...
{
    public static MField Instance { get; private set; }
    ...
    public static bool IsRotatable(MGame.EditMode editMode)
    {
        FieldType? fieldType = (FieldType?)MGame.TryConvertEnum<MGame.EditMode, FieldType>(editMode);
    ...
    public void SetField(int mx, int my, FieldType type, int rotation)
    {
        if (GetField(mx, my) == null || GetFieldType(GetField(mx, my)) != type)
        {
            ...
            if (MCoin.CantPlaceFields.Contains(type))
            {
            ...
            if (MKey.CantPlaceFields.Contains(type))
            {
                // TODO: 9x bad performance than before
                // remove key if wall is placed
                MGame.RemoveObjectInContainerIntersect(mx, my, MGame.Instance.KeyContainer);
            }
        }
    }
    [PunRPC]
    ...
    private static GameObject InstantiateField(Vector2 pos, FieldType type, int rotation)
    {
        ...
        {
            res = Instantiate(prefab, pos, Quaternion.Euler(0, 0, rotation), MGame.Instance.FieldContainer.transform);
        }
        return res;
    }
