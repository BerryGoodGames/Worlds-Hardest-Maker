...
/// manages game (duh)
/// </summary>
public class MGame : MonoBehaviourPun
{
    ...
    public EditMode CurrentEditMode
    {
        ...
        set
        {
            ...
            // update toolbar
            GameObject[] tools = MToolbar.tools;
            ...
            if (currentEditMode == EditMode.ANCHOR || currentEditMode == EditMode.BALL)
            {
                ...
                if (MAnchor.Instance.SelectedAnchor != null)
                {
                    // enable lines
                    MAnchor.Instance.selectedPathController.drawLines = true;
            ...
            else if (currentEditMode != EditMode.ANCHOR && currentEditMode != EditMode.BALL)
            {
                ...
                {
                    // disable lines
                    MAnchor.Instance.selectedPathController.drawLines = false;
                    MAnchor.Instance.selectedPathController.ClearLines();
    ...
    [HideInInspector] public int EditRotation
    {
        ...
        set
        {
            ...
            PlacementPreview.GetComponent<CPreview>().UpdateRotation();
        }
    }
    ...
    private void Start()
    {
        ...
        if (Instance.Multiplayer)
        {
            ...
            MPlayer.Instance.SetPlayer(0.5f, 0.5f, 3f);
        }
    ...
    private void LateUpdate()
    {
        ...
        try
        {
            CPlayer currentPlayer = MPlayer.GetPlayer().GetComponent<CPlayer>();
    ...
    {
        //foreach (KeyValuePair<int, CPlayer> pair in Instance.PlayerControllerList)
        ...
        MAudio.Instance.Play("Bell");
        MAudio.Instance.MusicFiltered(false);
        ...
        foreach (Transform coin in Instance.CoinContainer.transform)
        {
            ...
            anim.SetBool("Playing", true);
            anim.SetBool("PickedUp", coin.GetChild(0).GetComponent<CCoin>().pickedUp);
        }
        ...
        foreach (Transform key in Instance.KeyContainer.transform)
        {
            ...
            anim.SetBool("Playing", true);
            anim.SetBool("PickedUp", key.GetChild(0).GetComponent<CKey>().pickedUp);
        }
    ...
    public static void SwitchToEdit(bool playSoundEffect = true)
    {
        ...
        if(playSoundEffect) MAudio.Instance.Play("Bell");
        MAudio.Instance.MusicFiltered(true);
        ...
        if (MAnchor.Instance.selectedPathController != null)
        {
            // WTF
        }
        ...
        foreach (Transform coin in Instance.CoinContainer.transform)
        {
            coin.GetChild(0).GetComponent<CCoin>().pickedUp = false;
        ...
        foreach (Transform key in Instance.KeyContainer.transform)
        {
            key.GetChild(0).GetComponent<CKey>().pickedUp = false;
    ...
    {
        // reset players
        foreach (GameObject player in MPlayer.GetPlayers())
        {
            CPlayer controller = player.GetComponent<CPlayer>();
            if (Instance.Multiplayer && !controller.photonView.IsMine) continue;
        ...
        foreach (Transform ball in Instance.BallDefaultContainer.transform)
        {
            ...
            CBall controller = ballObject.GetComponent<CBall>();
        ...
        {
            GameObject ballObject = ball.GetChild(0).gameObject;
            CBallCircle controller = ballObject.GetComponent<CBallCircle>();
        ...
        foreach (Transform field in Instance.FieldContainer.transform)
        {
            if (field.CompareTag("CheckpointField"))
            {
                ...
                CCheckpoint controller = field.GetComponent<CCheckpoint>();
    ...
    {
        MPlayer.Instance.RemoveAllPlayers();
