...
/// </summary>
public class MFill : MonoBehaviour
{
    public static MFill Instance { get; private set; }

    public static readonly List<MGame.EditMode> NoFillPreviewModes = new(new MGame.EditMode[]
    {
        MGame.EditMode.BALL_DEFAULT,
        MGame.EditMode.BALL_CIRCLE,
        MGame.EditMode.GRAY_KEY,
        MGame.EditMode.RED_KEY,
        MGame.EditMode.BLUE_KEY,
        MGame.EditMode.GREEN_KEY,
        MGame.EditMode.YELLOW_KEY,
        MGame.EditMode.PLAYER
    });
    ...
    private void Update()
    {
        ...
        if (!MGame.Instance.Playing && MMouse.Instance.MouseDragStart != null && MMouse.Instance.MouseDragEnd != null && MGame.Instance.Filling)
        {
            // get drag positions and world position mode
            FollowMouse.WorldPosition worldPosition = MGame.Instance.CurrentEditMode.GetWorldPosition();

            (Vector2 start, Vector2 end) = MMouse.GetDragPositions(worldPosition);

            // disable normal placement preview
            MGame.Instance.PlacementPreview.SetActive(false);

            if (prevStart == null || !prevStart.Equals(start) || !prevEnd.Equals(end))
            {
                // reset fill marking
                foreach (Transform stroke in MGame.Instance.FillOutlineContainer.transform)
                {
                    Destroy(stroke.gameObject);
                }
                // reset fill previews
                foreach (Transform preview in MGame.Instance.FillPreviewContainer.transform)
                {
                ...
                MLine.DrawRect(
                    ...
                    alignCenter: false, parent: MGame.Instance.FillOutlineContainer.transform
                );

                // set new previews, only if editmode not in NoFillPreviewModes
                if (!NoFillPreviewModes.Contains(MGame.Instance.CurrentEditMode))
                {
                    foreach (Vector2 pos in fillRange)
                    {
                        Instantiate(MGame.Instance.FillPreview, pos, Quaternion.identity, MGame.Instance.FillPreviewContainer.transform);
                    }
                }

                MGame.Instance.CurrentFillRange = fillRange;
            }

            prevStart = (Vector2)MMouse.Instance.MouseDragStart;
            prevEnd = (Vector2)MMouse.Instance.MouseDragEnd;
        }
    }
    ...
    [PunRPC]
    public void FillArea(List<Vector2> poses, MField.FieldType type)
    {
        if (MGame.Instance.CurrentFillRange == null) return;
        MGame.Instance.CurrentFillRange = null;
        ...
        if (lowestX == highestX || lowestY == highestY)
        {
            ...
            {
                MField.Instance.SetField((int)pos.x, (int)pos.y, type, 0);
            }
            return;
        }
        ...
        foreach (Vector2 pos in poses)
        {
            MField.Instance.RemoveField((int)pos.x, (int)pos.y);
        }
        ...
        foreach (Vector2 pos in poses)
        {
            ...
            GameObject field = Instantiate(prefab, pos, Quaternion.Euler(0, 0, rotation), MGame.Instance.FieldContainer.transform);
            // REF
            ...
            if (!MPlayer.StartFields.Contains(type))
            {
                // TODO: 9x bad performance than before
                MPlayer.Instance.RemovePlayerAtPosIntersect(mx, my);
            }

            // remove coin if wall is placed
            if (type == MField.FieldType.WALL_FIELD)
            {
                // TODO: 9x bad performance than before
                MGame.RemoveObjectInContainerIntersect(mx, my, MGame.Instance.CoinContainer);
            }
        }
        ...
        if (type.GetPrefab().GetComponent<FieldOutline>() != null)
        {
            ...
            {
                if (MField.GetField(i, lowestY).TryGetComponent(out FOComp))
                    FOComp.UpdateOutline(Vector2.down, true);

                if (MField.GetField(i, highestY).TryGetComponent(out FOComp))
                    FOComp.UpdateOutline(Vector2.up, true);
            }

            for (int i = lowestY; i <= highestY; i++)
            {
                if (MField.GetField(lowestX, i).TryGetComponent(out FOComp))
                    FOComp.UpdateOutline(Vector2.left, true);

                if (MField.GetField(highestX, i).TryGetComponent(out FOComp))
    ...
    [PunRPC]
    public void FillArea(Vector2 start, Vector2 end, MField.FieldType type)
    {
    ...
    public static void ResetPreview()
    {
        ...
        {
            GameObject preview = MGame.Instance.PlacementPreview;
