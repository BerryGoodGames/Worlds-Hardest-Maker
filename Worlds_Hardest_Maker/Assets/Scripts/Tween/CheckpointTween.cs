...
public class CheckpointTween : MonoBehaviour, IResettable
{
    ...
    private static Color GetActiveColor()
    {
        ...
        return LevelSessionEditManager.Instance.IsPlaying && SettingsManager.Instance.OneColorSafeFields ? palette[5] : palette[3];
    }
    
    private static Color GetInactiveColor()
    {
        List<Color> palette = ColorPaletteManager.GetColorPalette("Start Goal Checkpoint").Colors;
        return LevelSessionEditManager.Instance.IsPlaying && SettingsManager.Instance.OneColorSafeFields ? palette[4] : palette[2];
    }
