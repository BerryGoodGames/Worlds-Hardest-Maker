...
public class MBallCircle : MonoBehaviour
{
    public static MBallCircle Instance { get; private set; }
    ...
    public List<GameObject> GetBallCircles(float mx, float my)
    {
        ...
        CBallCircle[] ballCircles = FindObjectsOfType<CBallCircle>();
        foreach(CBallCircle controller in ballCircles)
        {
    ...
    private static GameObject InstantiateBallCircle(Vector2 pos, float r, float speed, float startAngle)
    {
        ...
        {
            newBallCircle = Instantiate(MGame.Instance.BallCircle, Vector2.zero, Quaternion.identity, MGame.Instance.BallCircleContainer.transform);

            CBallCircle controller = newBallCircle.transform.GetChild(0).GetComponent<CBallCircle>();
    ...
    {
        GameObject container = MGame.Instance.BallCircleContainer;
        foreach (Transform bc in container.transform)
        {
            Vector2 originPos = bc.GetChild(0).GetComponent<CBallCircle>().origin.position;
