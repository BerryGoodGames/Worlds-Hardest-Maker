...
{
    public static MBall Instance { get; private set; }
    ...
    public void SetBall(float mx, float my, float bounceMx, float bounceMy, float speed)
    {
        ...
        if(!IsBallThere(mx, my))
        {
            ...
            // instantiate prefab
            InstantiateBall(pos, bouncePos, speed, MGame.Instance.Multiplayer);
        }
    }
    [PunRPC]
    ...
    public GameObject InstantiateBall(Vector2 pos, Vector2 bouncePos, float speed, bool multiplayer)
    {
        ...
        {
            newBall = Instantiate(MGame.Instance.BallDefault, Vector2.zero, Quaternion.identity, MGame.Instance.BallDefaultContainer.transform);

            CBall controller = newBall.transform.GetChild(0).GetComponent<CBall>();
    ...
    {
        //GameObject container = MGame.Instance.BallDefaultContainer;
        ...
        //    {
        //        hit.GetComponent<CBall>().DestroyBall();
        //        break;
        //    }
        //}

        CBall[] controllers = FindObjectsOfType<CBall>();
        foreach(CBall c in controllers)
        {
