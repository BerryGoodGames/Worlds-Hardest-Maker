...
public sealed class AnchorBlockTests
{
    ...
    public IEnumerator Smoke_PlaytestExecutesAtLeastOneFullLoop()
    {
        ...
        anchor.AppendBlock(new SetSpeedBlock(false, 1000f, MovementUnit.UnitsPerSecond));
    ...
    private AnchorController CreateAnchor()
    {
        ...
        AnchorController anchor = AnchorManager.Instance.CreateNew(config.anchorSpawnPosition, GlobalSheet.Instance);
        Assert.IsNotNull(
            anchor,
    ...
    private static void ReplaceBlocksWithAllExistingKinds(AnchorController anchor)
    {
        ...
        anchor.AppendBlock(new WaitBlock(false, 0.01f, TimeUnit.Seconds));
        anchor.AppendBlock(new SetEaseBlock(false, Ease.InBack));
        anchor.AppendBlock(new SetSpeedBlock(false, 5f, MovementUnit.UnitsPerSecond));
